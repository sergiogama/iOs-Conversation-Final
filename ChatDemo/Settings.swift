...
class Settings {
    ...
    /**
            Creates a Watson instance based on Settings
     **/
    static func makeWatson() -> Watson {
        var recognizer: SpeechRecognizer
        var recognitionApi: RecognitionApi
        var classificationApi: ClassificationApi
        var synthesisApi: SynthesisApi
        
        if Settings.useSpeechToText {
            recognitionApi = SpeechToTextApi(username: speechRecognitionUsername, password: speechRecognitionPassword, language: speechToTextLanguage)
        } else {
            recognitionApi = NativeRecognitionApi(recognizer: SFSpeechRecognizer(locale: Locale(identifier:nativeRecognitionLanguage))!)
        }
        recognizer = SpeechRecognizer(recognitionApi: recognitionApi, timerGroup: TimerGroup())
        
        classificationApi = ConversationApi(username: orchestratorUsername, password: orchestratorPassword, url: messageURL)
        
        if Settings.useNativeSynthesis {
            synthesisApi = NativeSynthesisApi(language: nativeSynthesisVoice)
        } else {
            synthesisApi = TextToSpeechApi(username: voiceSynthesisUsername, password: voiceSynthesisPassword, language: textToSpeechVoice)
        }
        
        return Watson(recognizer: recognizer, classificationApi: classificationApi, synthesisApi: synthesisApi)
    }
}
